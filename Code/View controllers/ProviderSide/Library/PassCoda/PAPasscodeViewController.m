    ...
    if (self) {
        ...
        switch (action) {
            ...
            case PasscodeActionChange:
                self.title = NSLocalizedString(@"Change PIN", nil);
...
- (void)showFailedAttempts {
    ...
    {
        failedAttemptsLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%d Failed PIN Attempts", nil), _failedAttempts];
    }
